...
export default function BillCalculator() {
  ...
  const [users, setUsers] = useState(['Tuhin', 'Kartic', 'Ashik', 'Sohag']);
  ...
  const calculateBills = () => {
    ...
    const divValue = eBillExtra / parseInt(divisions);
            ...
            <button
              onClick={() => {
                ...
                if (parseFloat(eBillTotal) > eBillRef && extraUsers.length === 0) {
                  setError('Please select extra users');
                  return;
                }
                setError('');
                setStep(2);
          ...
          <div className="space-y-6">
            ...
            {!divisions ? (
              <div>
                ...
                <button
                  onClick={() => {
                    if (!divisions || parseInt(divisions) <= 0) {
                      setError('Please enter a valid positive number');
                      return;
                    }
                    setError('');
              ...
              <div>
                ...
                {isLastUser ? (
                  <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-green-600 mt-1">
                      Amount: ₹{(remaining * (eBillExtra / parseInt(divisions))).toFixed(2)}
                    </p>
                  </div>
                ) : (
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      How many divisions for {extraUsers[currentUserIndex]}?
                    </label>
                    <input
                      type="number"
                      value={tempDivision}
                ...
                <button
                  ...
                  className="w-full mt-3 bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition-colors font-medium"
                >
                  {isLastUser ? 'View Summary →' : 'Next User →'}
                </button>
              </div>
            )}
          </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-gray-600">{month} 2024</p>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span>Base Electricity (per person):</span>
                <span className="font-medium">₹{results.eBillPrsn.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Water Bill (per person):</span>
                <span className="font-medium">₹{results.wBillPrsn.toFixed(2)}</span>
              </div>
              {wifiPayers.length > 0 && (
                <div className="flex justify-between text-sm">
                  <span>WiFi (per payer):</span>
                  <span className="font-medium">₹{results.wifiPerPayer.toFixed(2)}</span>
                </div>
              )}
              {results.eBillExtra > 0 && (
                <div className="flex justify-between text-sm">
                  <span>Extra Electricity (per division):</span>
                  <span className="font-medium">₹{results.divValue.toFixed(2)}</span>
                </div>
              )}
            </div>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 mt-2">Split bills fairly among roommates</p>
          </div>

          {step < 5 && (
            <div className="mb-8">
              <div className="flex justify-between mb-2">
                {['Electricity', 'Water', 'WiFi', 'Division'].map((label, idx) => (
                  <span key={idx} className={`text-xs font-medium ${step > idx + 1 ? 'text-blue-600' : 'text-gray-400'}`}>
                    {label}
                  </span>
                ))}
              </div>
              <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-blue-600 transition-all durati
